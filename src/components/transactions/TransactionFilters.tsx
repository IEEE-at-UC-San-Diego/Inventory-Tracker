      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={(e) => handleDateRangeChange(e.target.value as DateRange['value'])}
            ...
            {compartments.length > 0 && (
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateFilter('selectedCompartmentId', e.target.value || undefined)
                  }
                  className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 text-sm"
                >
...
function getUserName(users: Array<{ _id: string; name: string }>, userId: string): string {
...
export function filterTransactions(
  ...
  return transactions.filter((transaction) => {
    ...
    if (filters.selectedActions.length > 0) {
      if (!filters.selectedActions.includes(transaction.actionType)) return false
    }
    ...
    if (filters.selectedUserId) {
      if (transaction.userId !== filters.selectedUserId) return false
    }

    // Compartment filter (source or destination)
    if (filters.selectedCompartmentId) {
      const matchesSource =
        transaction.sourceCompartmentId === filters.selectedCompartmentId
      const matchesDest = transaction.destCompartmentId === filters.selectedCompartmentId
