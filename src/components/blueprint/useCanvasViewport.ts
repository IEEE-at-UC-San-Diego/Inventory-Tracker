...
import type { Viewport, CanvasPoint, CanvasBounds } from '@/types'
import {
  clampZoom,
  zoomAtPoint,
  getDefaultViewport,
...
export function useCanvasViewport({
  ...
  }, [drawers, containerWidth, containerHeight, minZoom, maxZoom])
      ...
      if (animate) {
        ...
        const animateViewport = (currentTime: number) => {
          ...
          setViewport({
            ...
            zoom: startViewport.zoom + (newZoom - startViewport.zoom) * easeProgress,
          })
  ...
  const startDrag = useCallback((point: CanvasPoint) => {
    setIsDragging(true)
  ...
  const drag = useCallback(
    (point: CanvasPoint) => {
      if (!isDragging || !dragStartRef.current || !viewportStartRef.current) return
