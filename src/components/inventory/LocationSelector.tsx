...
export function LocationSelector({ authContext, value, onChange, excludeCompartmentId }: LocationSelectorProps) {
  ...
  const availableCompartments = excludeCompartmentId
    ? compartments.filter((comp) => comp._id !== excludeCompartmentId)
    : compartments

  const selectedBlueprint = blueprints.find((b) => b._id === selectedBlueprintId)
  const selectedDrawer = drawers.find((d) => d._id === selectedDrawerId)
  const selectedCompartment = compartments.find((c) => c._id === value)
