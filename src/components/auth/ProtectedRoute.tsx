    ...
    return (
      <div className="flex h-screen flex-col items-center justify-center p-4">
        <div className="max-w-md rounded-lg border bg-card p-6 text-center shadow-sm">
          ...
          <p className="text-sm text-muted-foreground">
            Your current permissions: <strong>{user?.scopes?.join(', ') || 'None'}</strong>
          </p>
          <div className="mt-6">
            <a
              href="/dashboard"
...
export function ReadPermission({ children, fallback = null }: ReadPermissionProps) {
  return (
...
export function WritePermission({ children, fallback = null }: WritePermissionProps) {
  return (
...
export function EditorOnly({ children, fallback = null }: EditorOnlyProps) {
  return (
    <RoleGuard requiredRole="Executive Officers" fallback={fallback}>
      {children}
    </RoleGuard>
  )
}
