...
import { Loader2, RefreshCw, AlertTriangle, Clock, CheckCircle2 } from 'lucide-react'
...
interface RoleSyncQueueItem {
  _id: string
  _creationTime: number
  userId: string
  userName?: string // Populated from users table
  userEmail?: string
  targetRole: 'Administrator' | 'Executive Officers' | 'General Officers' | 'Member'
  attempts: number
...
export function RoleSyncQueueDialog({ open, onOpenChange }: RoleSyncQueueDialogProps) {
  const { toast } = useToast()
  const { authContext, getFreshAuthContext, isLoading } = useAuth()
  const [isRetrying, setIsRetrying] = useState<string | null>(null)

  // Fetch role sync queue items
  const queueResult = useQuery(api.role_sync_queue.queries.list, { authContext }, {
    enabled: !!authContext && !isLoading
  })
  const queueItems = queueResult || []
  ...
  const handleRetry = async (queueItemId: string) => {
    ...
    try {
      ...
      await api.role_sync_queue.mutations.retryRoleSync({
        authContext: context,
        queueItemId: queueItemId as any,
      })
      ...
      <Card className="w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-gray-900">Role Sync Queue</h2>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <div className="mt-2 text-xs text-gray-600">
                              <span className="font-medium">Target Role:</span> {item.targetRole}
                              <span className="ml-4 font-medium">Next attempt:</span>{' '}
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <div className="mt-2 text-xs text-gray-600">
                              <span className="font-medium">Target Role:</span> {item.targetRole}
                              <span className="ml-4 font-medium">Last attempt:</span>{' '}
