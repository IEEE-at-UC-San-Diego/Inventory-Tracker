...
const LOGTO_ENDPOINT = import.meta.env.VITE_LOGTO_ENDPOINT || 'https://your-logto-endpoint.logto.app'
...
export function setAuthContext(context: import('@/types/auth').AuthContext): void {
  if (typeof window === 'undefined') return

  try {
    localStorage.setItem(AUTH_STORAGE_KEYS.AUTH_CONTEXT, JSON.stringify(context))
  } catch {
 ...
 */
export async function verifyLogtoToken(_accessToken: string, _idTokenClaims: LogtoTokenClaims, _userInfo: LogtoUserInfo): Promise<{ success: boolean; user?: User; authContext?: AuthContext; tokenExpiresAt?: number; error?: string }> {
  console.log('[verifyLogtoToken] FUNCTION CALLED with token length:', _accessToken?.length)
  
  // DEBUG: Log token format before sending to API
  console.log('[verifyLogtoToken] Sending token to API:', {
    length: _accessToken?.length,
    startsWith: _accessToken?.substring(0, 50),
    segments: _accessToken?.split('.')?.length,
  })

  try {
    console.log('[verifyLogtoToken] Making API call to /api/verify-token')
    ...
    console.log('[verifyLogtoToken] API response status:', response.status)
    console.log('[verifyLogtoToken] API response ok:', response.ok)
    ...
    const data = (await response.json()) as { success: boolean; user?: User; authContext?: AuthContext; tokenExpiresAt?: number; error?: string }

    console.log('[verifyLogtoToken] API response:', data)
    ...
    console.error('[verifyLogtoToken] Error details:', {
 ...
 */
export async function logoutFromConvex(): Promise<{ success: boolean; error?: string }> {
