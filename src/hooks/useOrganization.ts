...
export function useOrganization() {
  const { user, isAuthenticated, authContext, isLoading: isAuthLoading } = useAuth()
  ...
  const orgData = useQuery(
    ...
    orgId ? { authContext, orgId: orgId as any } : 'skip'
  )
  ...
  const switchOrganization = useCallback(
    async (newOrgId: string): Promise<{ success: boolean; error?: string }> => {
      if (!isAuthenticated) {
...
export function useOrganizations() {
  ...
  const currentOrg = useQuery(
    api.organizations.queries.get,
    user?.orgId ? { authContext, orgId: user.orgId as any } : 'skip'
  )
