...
function InventoryContent() {
	...
	const partsResult = useQuery(
		...
		authContext ? { authContext, includeArchived: false } : undefined,
		{
			enabled: !!authContext && !isLoading,
		},
	);
	...
	const handleExportInventory = useCallback(() => {
		...
		toast.success(
			"Export Complete",
			`Downloaded ${filteredInventory.length} inventory items to CSV`,
		);
			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div>
					...
					<p className="text-gray-600 mt-1">
						Track parts across storage locations
					</p>
				</div>
				<div className="flex items-center gap-2">
					<Button
								...
								>
									<div
										className={`p-2 rounded-full ${
											transaction.actionType === "Add"
												? "bg-green-100 text-green-600"
												: transaction.actionType === "Remove"
													? "bg-red-100 text-red-600"
													: transaction.actionType === "Move"
