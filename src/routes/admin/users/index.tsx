...
import { api, fetchQuery } from 'convex/_generated/api'
...
type UserRole = 'Administrator' | 'Executive Officers' | 'General Officers' | 'Member'
...
function AdminUsersContent() {
  ...
  const syncQueueResult = useQuery(api.role_sync_queue.queries.getSummary, { authContext }, {
    enabled: !!authContext && !isLoading
  })
  const syncQueueSummary = syncQueueResult || { pending: 0, retry: 0, failed: 0, total: 0 }

  // Fetch individual sync statuses per user
  const [userSyncStatuses, setUserSyncStatuses] = useState<Map<string, 'pending' | 'retry' | 'failed'>>(new Map())
  const syncQueueItems = useQuery(api.role_sync_queue.queries.list, { authContext }, {
    enabled: !!authContext && !isLoading
  })
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        const result = await convex.query(api.auth_helpers.queries.getMyProfile, {})
  ...
  const roleCounts = useMemo(() => {
    const counts = { Administrator: 0, 'Executive Officers': 0, 'General Officers': 0, Member: 0 }
    users.forEach((user) => {
      counts[user.role]++
    })
    return counts
  ...
  const handleInviteMutation = async () => {
    try {
      ...
      await convex.mutation(api.organization_helpers.mutations.inviteUser, {
        authContext: context,
  ...
  const handleInvite = useCallback(async () => {
    ...
    try {
      ...
      toast.success('User invited', `${inviteForm.name} has been invited to the organization`)
  ...
  const handleRoleChange = useCallback(
    async (userId: string, newRole: UserRole) => {
      try {
        const context = await getFreshAuthContext() || authContext
        await convex.mutation(api.organization_helpers.mutations.updateUserRole, {
          authContext: context,
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      const context = await getFreshAuthContext() || authContext
      await convex.mutation(api.organization_helpers.mutations.removeUser, {
        authContext: context,
        userId: deletingUser._id as Id<'users'>,
      })
      toast.success('User removed', `${deletingUser.name} has been removed from the organization`)
  ...
  const handleExportUsers = useCallback(() => {
    ...
    toast.success('Export Complete', `Downloaded ${filteredUsers.length} users to CSV`)
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-1">{roleCounts.Admin}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-1">{roleCounts.Editor}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-1">{roleCounts.Member}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-1">{roleCounts.Viewer}</p>
              </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-1" title={`Role sync status: ${userSyncStatuses.get(user._id) || 'synced'}`}>
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    The user will receive an invitation via Logto. They can sign in and access the organization based on their assigned role.
                  </p>
                </div>
              </div>
                      ...
                      <SelectItem value="Editor">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">Can edit inventory</p>
                          </div>
                        </div>
                      </SelectItem>
                      <SelectItem value="Member">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">Check in/out items</p>
                          </div>
                        </div>
                      </SelectItem>
