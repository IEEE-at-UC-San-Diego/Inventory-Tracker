...
function SettingsContent() {
	...
	const orgResult = useQuery(
		...
		authContext
			? { authContext, id: authContext.orgId as Id<"organizations"> }
			: undefined,
		{
	...
	const handleSave = useCallback(async () => {
		...
		try {
			...
			await convex.mutation(api.organizations.mutations.update, {
				authContext: context,
				orgId: orgResult._id as Id<"organizations">,
				name: form.orgName,
			});
	...
	const getEnvironmentInfo = () => {
		const packageInfo = packageJson as {
			version?: string;
			description?: string;
		};
		return {
			...
			"Build Date": packageInfo.description || "Unknown",
		};
	};
							...
							{orgResult && (
								<div className="p-3 bg-cyan-50 rounded-lg border border-cyan-200">
									<div className="flex items-center gap-2 text-sm text-cyan-800">
										...
										<span>
											Organization ID:{" "}
											<code className="font-mono bg-cyan-100 px-1 rounded">
												{orgResult._id}
											</code>
										</span>
									</div>
									...
									<div>
										...
										<div className="mt-1.5">
											<Badge
												className={cn(
													"border",
													getRoleBadgeColor(profileResult.user.role),
												)}
											>
