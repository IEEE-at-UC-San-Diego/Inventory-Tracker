...
import type { VerifyTokenResponse, AuthContext, UserRole } from '@/types/auth'
...
const API_RESOURCE =
	getEnv('LOGTO_API_RESOURCE') || getEnv('VITE_LOGTO_API_RESOURCE') || 'urn:inventory-tracker:api'

// Convex URL for user sync - required
const CONVEX_URL = getEnv('CONVEX_SELF_HOSTED_URL') || getEnv('VITE_CONVEX_URL')
...
async function syncUserToConvex(
	...
	role: UserRole
): Promise<{ _id: string; logtoUserId: string; name: string; email: string; orgId: string; role: UserRole; createdAt: number }> {
	if (!CONVEX_URL) {
		...
		handlers: {
			POST: async ({ request }): Promise<Response> => {
				...
				try {
					...
					console.log('[verify-token] Raw accessToken length:', accessToken?.length)
					console.log('[verify-token] Raw accessToken starts with:', accessToken?.substring(0, 50))
					console.log('[verify-token] Raw accessToken ends with:', accessToken?.slice(-50))
					...
					if (segments.length !== 3) {
						console.error('[verify-token] Invalid token format: expected 3 segments, got', segments.length)
						const response = new Response(
							JSON.stringify({
								success: false,
					...
					// DEBUG: Log the cleaned token
					console.log('[verify-token] CleanToken length:', cleanToken?.length)
					console.log('[verify-token] CleanToken starts with:', cleanToken?.substring(0, 50))
					console.log('[verify-token] CleanToken ends with:', cleanToken?.slice(-50))
					console.log('[verify-token] CleanToken segments:', cleanToken?.split('.')?.length)
					...
					const name = (payload.name as string) || (email ? email.split('@')[0] : 'Unknown')

					// Debug: Log the full payload to understand the structure
					console.log('[verify-token] JWT payload:', JSON.stringify(payload, null, 2))
					...
					if (Array.isArray(payload.roles)) {
						...
						extractedRoles = payload.roles.map((r: unknown) =>
					...
					if (payload.user && typeof payload.user === 'object') {
						...
						if (userObj.roles && Array.isArray(userObj.roles)) {
							extractedScopes = userObj.roles
								.flatMap((role) => role.scopes?.map((scope) => scope.name) || [])
								.filter(Boolean)
						}
					}
					...
					console.log('[verify-token] Extracted organizations:', extractedOrganizations)
					...
					const role: UserRole = validRoles.includes(roleClaim) ? (roleClaim as UserRole) : 'Member'

					console.log('[verify-token] Determined role:', role, 'from roleClaim:', roleClaim)
					...
					try {
						user = await syncUserToConvex(logtoUserId, email, name, orgIdClaim, role)
					} catch (error) {
						console.error('[verify-token] Failed to sync user to Convex:', error)
						const response = new Response(
							JSON.stringify({
								success: false,
					...
					try {
						const orgResponse = await fetch(`${CONVEX_URL}/http/api/organizations/${user.orgId}`, {
							method: 'GET',
							headers: {
								'Content-Type': 'application/json',
							},
