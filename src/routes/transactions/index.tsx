...
function TransactionsContent() {
  ...
  // Fetch stats
  const statsResult = useQuery(api["transactions/queries"].getStats, { authContext }, {
    enabled: !!authContext && !isLoading
  })

  // Fetch users for filter dropdown
  const orgUsers = useQuery(api["organizations/queries"].getOrgMembers, 
    authContext ? { authContext, organizationId: authContext.orgId } : 'skip', {
    enabled: !!authContext && !isLoading
  })

  // Fetch compartments for location filter
  const inventoryResult = useQuery(api["inventory/queries"].list, { authContext }, {
    enabled: !!authContext && !isLoading
  })
  ...
  const compartments = useMemo(() => {
    ...
    inventoryResult?.forEach((item) => {
      if (item.compartment) {
        uniqueComps.set(item.compartment._id, item.compartment)
      }
    })
  ...
  const handleExport = useCallback(() => {
    ...
    toast.success('Export Complete', `Downloaded ${filteredTransactions.length} transactions to CSV`)
  ...
  const actionStats = useMemo(() => {
    ...
    const removes = transactions.filter((t) => t.actionType === 'Remove').length
    const moves = transactions.filter((t) => t.actionType === 'Move').length
    const adjusts = transactions.filter((t) => t.actionType === 'Adjust').length
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        <ActionStatCard
          ...
          color="red"
          trend={stats ? `-${stats.transactionsByType.Remove} today` : undefined}
        />
        <ActionStatCard
          title="Moves"
      ...
      <Card>
        <CardContent className="p-4">
          <TransactionFilters
            ...
            onExport={filteredTransactions.length > 0 ? handleExport : undefined}
...
function ActionStatCard({ title, value, icon, color, trend }: ActionStatCardProps) {
  const colorClasses = {
    green: {
      bg: 'bg-green-50',
      text: 'text-green-700',
  ...
  return (
    <div className={`p-4 rounded-lg border ${colors.bg} ${colors.border}`}>
      ...
      <p className={`text-2xl font-bold mt-1 ${colors.text}`}>{value.toLocaleString()}</p>
      {trend && <p className={`text-xs mt-1 ${colors.text} opacity-75`}>{trend}</p>}
    </div>
  )
}
