...
import { Plus, Folder, Lock, Unlock, Users, Settings, Edit } from 'lucide-react'
...
import type { BlueprintWithHierarchy } from '@/types'
...
function BlueprintsContent() {
  ...
  const blueprints = (blueprintsResult as BlueprintWithHierarchy[]) ?? []

  // Calculate stats
  const totalBlueprints = blueprints.length
  const lockedBlueprints = blueprints.filter((bp) => bp.lockedAt !== null).length
  const unlockedBlueprints = totalBlueprints - lockedBlueprints
  const totalDrawers = blueprints.reduce((sum, bp) => sum + (bp.drawers?.length || 0), 0)
    ...
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardContent className="p-0">
          {isLoadingBlueprints ? (
            <div className="p-12 text-center text-gray-500">Loading blueprints...</div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-4 mt-1 text-sm text-gray-500">
                          <span className="flex items-center gap-1">
                            ...
                            {blueprint.drawers?.length || 0} drawers
                          </span>
                          <span className="flex items-center gap-1">
