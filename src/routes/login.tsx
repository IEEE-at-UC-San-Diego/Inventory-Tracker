        ...
        {!isLoading && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-3">
              <DevLoginButton role="Viewer" label="Read-only" />
              <DevLoginButton role="Member" label="Check in/out" />
              <DevLoginButton role="Editor" label="Can edit" />
              <DevLoginButton role="Admin" label="Full access" />
            </div>

            <p className="text-xs text-center text-gray-500">
              Development login allows testing different user roles without Logto configuration.
            </p>
          </div>
        )}
...
function DevLoginButton({ role, label }: { role: UserRole; label: string }) {
  const handleDevLogin = async () => {
    ...
    localStorage.setItem('inventory_tracker_convex_user', JSON.stringify(devUser))
