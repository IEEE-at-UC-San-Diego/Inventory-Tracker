...
import { Plus, Package, Archive, Filter, LayoutGrid, List as ListIcon, MapPin, Download } from 'lucide-react'
import { useQuery, useMutation } from '@/integrations/convex/react-query'
import { api } from '../../../convex/_generated/api'
import { ProtectedRoute, EditorOnly } from '@/components/auth/ProtectedRoute'
...
function PartsContent() {
  ...
  const inventoryResult = useQuery(api["inventory/queries"].list, {
    authContext,
    includeDetails: true,
  }, {
    enabled: !!authContext && !isLoading
  ...
  const partsWithStats = useMemo(() => {
    const inventoryByPart = new Map<string, { quantity: number; locations: number }>()
    ...
    inventory.forEach((item) => {
      ...
      if (existing) {
        ...
        inventoryByPart.set(item.partId, { quantity: item.quantity, locations: 1 })
      }
    })

    return parts.map((part) => {
      const stats = inventoryByPart.get(part._id) ?? { quantity: 0, locations: 0 }
      return {
        ...part,
  ...
  const handleArchive = useCallback(async (partId: string, archived: boolean) => {
    try {
  ...
  }, [archivePart, unarchivePart, toast, authContext, getAuthContextForMutation])
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      const context = await getAuthContextForMutation(authContext)
      await deletePartMutation({ authContext: context, partId: deletePart._id as Id<'parts'> })
  ...
  }, [deletePart, deletePartMutation, toast, authContext, getAuthContextForMutation])
  ...
  const handleExportParts = useCallback(() => {
    ...
    toast.success('Export Complete', `Downloaded ${filteredParts.length} parts to CSV`)
  }, [filteredParts, toast])
  ...
  const totalInventory = partsWithStats.reduce((sum, p) => sum + (p.totalQuantity ?? 0), 0)
