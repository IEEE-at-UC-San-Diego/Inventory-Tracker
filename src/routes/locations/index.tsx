      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-4 bg-white rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-600">Blueprints</span>
            <MapPin className="w-4 h-4 text-cyan-600" />
          </div>
          <p className="text-2xl font-bold mt-1 text-gray-900">{blueprints.length}</p>
        </div>
        <div className="p-4 bg-white rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-600">Compartments</span>
            <Layers className="w-4 h-4 text-cyan-600" />
          </div>
          <p className="text-2xl font-bold mt-1 text-gray-900">{totalCompartments}</p>
        </div>
        <div className="p-4 bg-white rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-600">Total Parts</span>
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-medium text-gray-900">Filter by Blueprint</span>
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-900">
                        {compartment.compartmentLabel || `Compartment ${compartment.compartmentId.slice(-6)}`}
                      </h3>
                      <p className="text-sm text-gray-500">
                        {compartment.items.length} part
                        {compartment.items.length !== 1 ? 's' : ''} in this location
                      </p>
                    </div>
                  </div>
                  <Badge variant="outline" className="text-sm">
                    {compartment.items.reduce((sum, item) => sum + item.quantity, 0)} total
                    items
                  </Badge>
                </div>
                  ...
                  {compartment.items.map((item, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-gray-900 text-sm">{item.partName}</p>
