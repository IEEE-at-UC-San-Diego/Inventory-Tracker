...
import { verifyLogtoToken, setConvexUser, setTokenExpiresAt, setAuthContext, dispatchAuthUpdatedEvent, AUTH_UPDATED_EVENT, getConvexUser, getAuthContext } from '../lib/auth'
...
function CallbackPage() {
  ...
  const { getAccessToken, getIdTokenClaims, fetchUserInfo, isAuthenticated: logtoAuthenticated } = useLogto()
  ...
  useEffect(() => {
    const handleCallback = async () => {
      const waitForAuthProviderHydration = async () => {
        ...
        await new Promise<void>((resolve) => {
          ...
          window.addEventListener(AUTH_UPDATED_EVENT, handleAuthUpdated, { once: true })
      ...
      try {
        ...
        if (!isLoading && logtoAuthenticated) {
          ...
          if (result.success && result.user && result.authContext) {
            ...
            console.log('[callback] Wrote user to localStorage:', result.user._id)

            // Store auth context to trigger immediate state refresh
            setAuthContext(result.authContext)
            console.log('[callback] Wrote authContext to localStorage:', result.authContext.userId)
            ...
            console.log('[callback] Verified localStorage contents:', {
              user: verifyUser ? { _id: verifyUser._id, role: verifyUser.role } : null,
            ...
            setStatus('error')
            setErrorMessage(result.error || 'Failed to authenticate with backend')
            setTimeout(() => navigate({ to: '/login' }), 3000)
          }
        }
      } catch (err) {
    ...
    handleCallback()
  }, [isLoading, error, logtoAuthenticated, navigate, search, getAccessToken, getIdTokenClaims, fetchUserInfo])
